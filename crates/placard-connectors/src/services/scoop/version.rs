...
pub(crate) fn resolve_version(
    ...
    let app = params
        .get("app")
        .ok_or("scoop-version requires a data-app attribute")?;
    ...
    let version = value
        .get("version")
        .ok_or("scoop app response missing version")?;
    version
        .as_text()
...
mod tests {
    ...
    #[test]
    fn extracts_the_version_from_the_default_bucket() {
        ...
        let value = resolve_version(&params("ngrok"), &fetcher).unwrap();
    ...
    fn errors_when_version_field_is_missing() {
        let fetcher = FakeFetcher {
            buckets_response: BUCKETS,
            app_response: r#"{"license": "MIT"}"#,
            expected_app_url: "https://raw.githubusercontent.com/ScoopInstaller/Main/master/bucket/ngrok.json",
            calls: AtomicUsize::new(0),
        };
        assert!(resolve_version(&params("ngrok"), &fetcher).is_err());
    }
}
