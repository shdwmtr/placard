...
pub(crate) fn resolve_license(
    ...
    let channel = params
        .get("channel")
        .ok_or("conda-license requires a data-channel attribute")?;
    let package = params
        .get("package")
        .ok_or("conda-license requires a data-package attribute")?;
    ...
    let license = value
        .get("license")
        .ok_or("conda response missing license")?;
    license
        .as_text()
...
mod tests {
    ...
    impl Fetcher for FakeFetcher {
        fn fetch(&self, url: &str) -> Result<Vec<u8>, String> {
            assert_eq!(
                url,
                "https://api.anaconda.org/package/conda-forge/mlforecast"
            );
    ...
    fn extracts_the_license_field() {
        let fetcher = FakeFetcher(r#"{"license": "Apache-2.0"}"#);
        let value = resolve_license(&params("conda-forge", "mlforecast"), &fetcher).unwrap();
    ...
    fn requires_channel_and_package_params() {
        ...
        assert!(resolve_license(&params("conda-forge", ""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_params_before_fetching() {
        ...
        assert!(resolve_license(&params("../etc", "mlforecast"), &Unused).is_err());
    }

    #[test]
    fn errors_when_the_field_is_missing() {
        let fetcher = FakeFetcher(r#"{"latest_version": "1.0"}"#);
        assert!(resolve_license(&params("conda-forge", "mlforecast"), &fetcher).is_err());
    }
}
