...
pub(crate) fn resolve_coverage(
    ...
    let vcs = params
        .get("vcs")
        .ok_or("scrutinizer-coverage requires a data-vcs attribute")?;
    let user = params
        .get("user")
        .ok_or("scrutinizer-coverage requires a data-user attribute")?;
    let repo = params
        .get("repo")
        .ok_or("scrutinizer-coverage requires a data-repo attribute")?;
    ...
    let branch_info = obj_get(applications, &branch_name)
        .ok_or_else(|| format!("no coverage info for branch '{branch_name}'"))?;
    ...
    let raw = obj_get(metric_values, "scrutinizer.test_coverage")
        .ok_or("scrutinizer response missing scrutinizer.test_coverage")?;
    let raw_coverage = match raw {
        Value::Number(n) => *n,
        _ => return Err("scrutinizer.test_coverage was not a number".to_string()),
    };
    if raw_coverage <= 0.0 {
        return Err("coverage not found".to_string());
    }

    let coverage = raw_coverage * 100.0;
...
mod tests {
    ...
    #[test]
    fn extracts_coverage_percentage_for_the_default_branch() {
        let fetcher = FakeFetcher {
            expected_url: "https://scrutinizer-ci.com/api/repositories/g/filp/whoops",
            body: r#"{"default_branch": "master", "applications": {"master": {"index": {"_embedded": {"project": {"metric_values": {"scrutinizer.test_coverage": 0.7623}}}}}}}"#,
        };
        let value = resolve_coverage(&params("g", "filp", "whoops"), &fetcher).unwrap();
        assert_eq!(value, "76%");
    }
    ...
    fn requires_vcs_user_and_repo_params() {
        ...
        assert!(resolve_coverage(&params("g", "filp", ""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_params_before_fetching() {
        ...
        assert!(resolve_coverage(&params("g", "../etc", "whoops"), &Unused).is_err());
    }
    ...
    fn errors_when_metrics_are_missing_for_the_branch() {
        ...
        assert!(resolve_coverage(&params("g", "filp", "whoops"), &fetcher).is_err());
    }
}
