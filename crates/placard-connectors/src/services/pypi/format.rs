...
pub(crate) fn resolve_format(
    ...
    let package = params
        .get("package")
        .ok_or("pypi-format requires a data-package attribute")?;
    ...
    let has_egg = items.iter().any(|item| {
        matches!(
            item.get("packagetype").and_then(|v| v.as_text()).as_deref(),
            Some("egg") | Some("bdist_egg")
        )
    });
...
mod tests {
    ...
    #[test]
    fn reports_wheel_when_a_wheel_is_present() {
        let fetcher =
            FakeFetcher(r#"{"urls": [{"packagetype": "sdist"}, {"packagetype": "bdist_wheel"}]}"#);
        let value = resolve_format(&params("requests"), &fetcher).unwrap();
        assert_eq!(value, "wheel");
    }

    #[test]
    fn reports_egg_when_only_an_egg_is_present() {
        let fetcher = FakeFetcher(r#"{"urls": [{"packagetype": "bdist_egg"}]}"#);
        let value = resolve_format(&params("requests"), &fetcher).unwrap();
        assert_eq!(value, "egg");
    }

    #[test]
    fn reports_source_when_neither_is_present() {
    ...
    fn requires_package_param() {
        ...
        assert!(resolve_format(&HashMap::new(), &Unused).is_err());
        assert!(resolve_format(&params(""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_params_before_fetching() {
        ...
        assert!(resolve_format(&params("../etc"), &Unused).is_err());
    }

    #[test]
    fn errors_when_urls_field_is_missing() {
        let fetcher = FakeFetcher(r#"{}"#);
        assert!(resolve_format(&params("requests"), &fetcher).is_err());
    }
}
