...
pub(crate) fn resolve_downloads(
    ...
    let resource_id = params
        .get("resource-id")
        .ok_or("voxelshop-downloads requires a data-resource-id attribute")?;
    ...
    let downloads = value
        ...
        .ok_or("voxelshop response missing response.resource.downloads")?;
    downloads
        .as_text()
...
mod tests {
    ...
    #[test]
    fn extracts_downloads_from_a_voxelshop_shaped_response() {
        let fetcher = FakeFetcher(
            r#"{"response":{"resource":{"downloads":48213,"reviews":{"count":10,"stars":4.5},"updates":{"latest":{"version":"1.2.3"}}}}}"#,
        );
