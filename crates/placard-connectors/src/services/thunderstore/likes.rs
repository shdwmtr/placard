...
pub(crate) fn resolve_likes(
    ...
    let namespace = params
        .get("namespace")
        .ok_or("thunderstore-likes requires a data-namespace attribute")?;
    let namespace = validate_path_param("namespace", namespace)?;
    let package_name = params
        .get("package-name")
        .ok_or("thunderstore-likes requires a data-package-name attribute")?;
    ...
    let likes = value
        .get("rating_score")
        .ok_or("thunderstore response missing rating_score")?;
    likes
        .as_text()
        .ok_or_else(|| "rating_score was not a plain value".to_string())
}
...
mod tests {
    ...
    #[test]
    fn extracts_rating_score_from_package_metrics() {
        let fetcher =
            FakeFetcher(r#"{"downloads": 15234, "rating_score": 42, "latest_version": "1.2.3"}"#);
        let value = resolve_likes(&params("notnotnotswipez", "MoreCompany"), &fetcher).unwrap();
        assert_eq!(value, "42");
    }
    ...
    fn requires_namespace_and_package_name_params() {
        ...
        assert!(resolve_likes(&params("", "MoreCompany"), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_params() {
        ...
        assert!(resolve_likes(&params("notnotnotswipez", "a/b"), &Unused).is_err());
    }

    #[test]
    fn errors_when_the_field_is_missing() {
        let fetcher = FakeFetcher(r#"{"downloads": 15234, "latest_version": "1.2.3"}"#);
        assert!(resolve_likes(&params("notnotnotswipez", "MoreCompany"), &fetcher).is_err());
    }
}
