...
pub(crate) fn resolve_test_count(
    params: &HashMap<String, String>,
    fetcher: &dyn Fetcher,
) -> Result<String, String> {
    let metric = params
        .get("metric")
        .ok_or("testspace-test-count requires a data-metric attribute")?;
    if !matches!(
        metric.as_str(),
        "total" | "passed" | "failed" | "skipped" | "errored" | "untested"
    ) {
        return Err(
            "'metric' parameter must be one of total, passed, failed, skipped, errored, untested"
                .to_string(),
        );
    }
    let org = params
        .get("org")
        .ok_or("testspace-test-count requires a data-org attribute")?;
    let org = validate_org(org)?;
    let project = params
        .get("project")
        .ok_or("testspace-test-count requires a data-project attribute")?;
    let project = validate_project(project)?;
    let space = params
        .get("space")
        .ok_or("testspace-test-count requires a data-space attribute")?;
...
mod tests {
    ...
    fn params(metric: &str, org: &str, project: &str, space: &str) -> HashMap<String, String> {
        HashMap::from([
    ...
    #[test]
    fn extracts_the_metric_matching_case_counts() {
        let fetcher = FakeFetcher {
            expected_url: "https://swellaby.testspace.com/api/projects/swellaby%3Atestspace-sample/spaces/main/results",
            body: r#"[{"case_counts": [10, 2, 1, 0, 3]}]"#,
        };
        let value = resolve_test_count(
            &params("passed", "swellaby", "swellaby:testspace-sample", "main"),
            &fetcher,
        )
        .unwrap();
        assert_eq!(value, "10");
    ...
    #[test]
    fn sums_all_counts_for_total_metric() {
        let fetcher = FakeFetcher {
            expected_url: "https://swellaby.testspace.com/api/projects/proj/spaces/main/results",
            body: r#"[{"case_counts": [10, 2, 1, 0, 3]}]"#,
        };
        let value =
            resolve_test_count(&params("total", "swellaby", "proj", "main"), &fetcher).unwrap();
        assert_eq!(value, "16");
    }
    ...
    fn requires_all_params() {
        ...
        assert!(resolve_test_count(&HashMap::new(), &Unused).is_err());
        assert!(resolve_test_count(&params("bogus", "org", "proj", "space"), &Unused).is_err());
        assert!(resolve_test_count(&params("passed", "", "proj", "space"), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_params() {
        ...
        assert!(
            resolve_test_count(&params("passed", "org/../x", "proj", "space"), &Unused).is_err()
        );
    }

    #[test]
    fn errors_when_the_space_has_no_results() {
        let fetcher = FakeFetcher {
            expected_url: "https://swellaby.testspace.com/api/projects/proj/spaces/main/results",
            body: r#"[]"#,
        };
        assert!(
            resolve_test_count(&params("passed", "swellaby", "proj", "main"), &fetcher).is_err()
        );
    }
}
