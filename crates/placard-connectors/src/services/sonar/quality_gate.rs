...
pub(crate) fn resolve_quality_gate(
    params: &HashMap<String, String>,
    fetcher: &dyn Fetcher,
) -> Result<String, String> {
    let component = params
        .get("component")
        .ok_or("sonar-quality-gate requires a data-component attribute")?;
    let component = validate_component(component)?;
    let server = params
        .get("server")
        .ok_or("sonar-quality-gate requires a data-server attribute")?;
    let server = validate_server(server)?;

    let mut url = format!(
        "{server}/api/measures/component?component={}&metricKeys=alert_status",
    ...
    let quality_state = find_measure(&value, "alert_status")
        .ok_or("sonar response missing alert_status measure")?;
    Ok(if quality_state == "OK" {
        "passed".to_string()
    } else {
        "failed".to_string()
    })
...
mod tests {
    ...
    #[test]
    fn renders_passed_when_alert_status_is_ok() {
        let fetcher = FakeFetcher {
            expected_url: "https://sonarcloud.io/api/measures/component?component=michelin_kstreamplify&metricKeys=alert_status",
            body: r#"{"component": {"measures": [{"metric": "alert_status", "value": "OK"}]}}"#,
        };
        let value = resolve_quality_gate(
    ...
    #[test]
    fn renders_failed_for_any_other_alert_status() {
        let fetcher = FakeFetcher {
            expected_url: "https://sonarcloud.io/api/measures/component?component=michelin_kstreamplify&metricKeys=alert_status",
            body: r#"{"component": {"measures": [{"metric": "alert_status", "value": "ERROR"}]}}"#,
        };
        let value = resolve_quality_gate(
            &params("michelin_kstreamplify", "https://sonarcloud.io"),
            &fetcher,
    ...
    fn appends_branch_to_the_query_when_provided() {
        let mut p = params("michelin_kstreamplify", "https://sonarcloud.io");
        p.insert("branch".to_string(), "main".to_string());
        let fetcher = FakeFetcher {
            expected_url: "https://sonarcloud.io/api/measures/component?component=michelin_kstreamplify&metricKeys=alert_status&branch=main",
            body: r#"{"component": {"measures": [{"metric": "alert_status", "value": "OK"}]}}"#,
        };
        let value = resolve_quality_gate(&p, &fetcher).unwrap();
        assert_eq!(value, "passed");
    }
    ...
    fn requires_component_and_server_params() {
        ...
        assert!(resolve_quality_gate(&HashMap::new(), &Unused).is_err());
        assert!(resolve_quality_gate(&params("", "https://sonarcloud.io"), &Unused).is_err());
        assert!(resolve_quality_gate(&params("comp", ""), &Unused).is_err());
    }
    ...
    fn rejects_disallowed_component_and_server_values() {
        ...
        assert!(resolve_quality_gate(&params("a?b=c", "https://sonarcloud.io"), &Unused).is_err());
        assert!(resolve_quality_gate(&params("comp", "not-a-url"), &Unused).is_err());
    }

    #[test]
    fn errors_when_the_measure_is_missing() {
        let fetcher = FakeFetcher {
            expected_url: "https://sonarcloud.io/api/measures/component?component=comp&metricKeys=alert_status",
            body: r#"{"component": {"measures": []}}"#,
        };
        assert!(resolve_quality_gate(&params("comp", "https://sonarcloud.io"), &fetcher).is_err());
    }
}
