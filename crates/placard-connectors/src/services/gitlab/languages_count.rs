...
pub(crate) fn resolve_languages_count(
    ...
    let project = params
        .get("project")
        .ok_or("gitlab-languages-count requires a data-project attribute")?;
    let project = validate_project(project)?;
    let base = base_url(params);

    let url = format!(
        "{base}/api/v4/projects/{}/languages",
    ...
    let Value::Object(fields) = value else {
        return Err("gitlab response was not a JSON object".to_string());
    };
...
mod tests {
    ...
    #[test]
    fn counts_the_languages_in_a_gitlab_shaped_response() {
        let fetcher = FakeFetcher {
            expected_url: "https://gitlab.com/api/v4/projects/gitlab-org%2Fgitlab/languages",
            body: r#"{"Ruby": 67.13, "JavaScript": 19.66, "Vue": 13.21}"#,
        };
        let value = resolve_languages_count(&params("gitlab-org/gitlab"), &fetcher).unwrap();
        assert_eq!(value, "3");
    }
    ...
    fn requires_project_param() {
        ...
        assert!(resolve_languages_count(&params(""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_project_params() {
        ...
        assert!(resolve_languages_count(&params("../etc/passwd"), &Unused).is_err());
    }

    #[test]
    fn errors_when_the_response_is_not_an_object() {
        let fetcher = FakeFetcher {
            expected_url: "https://gitlab.com/api/v4/projects/gitlab-org%2Fgitlab/languages",
            body: r#"[]"#,
        };
        assert!(resolve_languages_count(&params("gitlab-org/gitlab"), &fetcher).is_err());
    }
}
