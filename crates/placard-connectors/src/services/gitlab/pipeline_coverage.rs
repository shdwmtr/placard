...
fn validate_job_name(value: &str) -> Result<&str, String> {
    if value.is_empty() {
        return Err("'job-name' parameter must not be empty".to_string());
    }
    if !value
        .chars()
        .all(|c| c.is_ascii_alphanumeric() || c == '-' || c == '_' || c == '.')
    {
        return Err("'job-name' parameter contains disallowed characters".to_string());
    }
    Ok(value)
}
...
pub(crate) fn resolve_pipeline_coverage(
    params: &HashMap<String, String>,
    fetcher: &dyn Fetcher,
) -> Result<String, String> {
    let project = params
        .get("project")
        .ok_or("gitlab-pipeline-coverage requires a data-project attribute")?;
    let project = validate_project(project)?;
    let branch = params
        .get("branch")
        .ok_or("gitlab-pipeline-coverage requires a data-branch attribute")?;
    let branch = validate_branch(branch)?;
    let base_url = resolve_base_url(params)?;

    let mut url = format!("{base_url}/{project}/badges/{branch}/coverage.svg");
    if let Some(job_name) = params.get("job-name") {
        let job_name = validate_job_name(job_name)?;
        url.push_str("?job=");
    ...
    if coverage == "unknown" {
        return Err("coverage not set up".to_string());
    }
    Ok(coverage)
}
...
mod tests {
    ...
    fn params(project: &str, branch: &str) -> HashMap<String, String> {
        HashMap::from([
    ...
    #[test]
    fn extracts_the_coverage_percentage_from_the_scraped_svg() {
        let body = badge_svg("85.71%");
        let fetcher = FakeFetcher {
            expected_url: "https://gitlab.com/gitlab-org/gitlab/badges/master/coverage.svg",
            body: body.clone(),
        };
        let value =
            resolve_pipeline_coverage(&params("gitlab-org/gitlab", "master"), &fetcher).unwrap();
        assert_eq!(value, "85.71%");
    }

    #[test]
    fn appends_job_name_and_uses_a_custom_gitlab_url_when_provided() {
        let mut p = params("gitlab-org/gitlab", "master");
        p.insert("job-name".to_string(), "jest-integration".to_string());
        p.insert(
            "gitlab-url".to_string(),
            "https://gitlab.example.com".to_string(),
        );
        let body = badge_svg("42%");
        let fetcher = FakeFetcher {
            expected_url: "https://gitlab.example.com/gitlab-org/gitlab/badges/master/coverage.svg?job=jest-integration",
            body: body.clone(),
        };
        let value = resolve_pipeline_coverage(&p, &fetcher).unwrap();
        assert_eq!(value, "42%");
    }

    #[test]
    fn requires_project_and_branch_params() {
        struct Unused;
        ...
        assert!(resolve_pipeline_coverage(&HashMap::new(), &Unused).is_err());
        assert!(resolve_pipeline_coverage(&params("gitlab-org/gitlab", ""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_params_before_fetching() {
        ...
        assert!(resolve_pipeline_coverage(&params("../etc/passwd", "master"), &Unused).is_err());
    }

    #[test]
    fn errors_when_coverage_is_not_set_up() {
        ...
        assert!(
            resolve_pipeline_coverage(&params("gitlab-org/gitlab", "master"), &fetcher).is_err()
        );
    }
}
