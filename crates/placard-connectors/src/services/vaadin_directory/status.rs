...
pub(crate) fn resolve_status(
    ...
    let package_name = params
        .get("package-name")
        .ok_or("vaadin-directory-status requires a data-package-name attribute")?;
    ...
    let status = value
        .get("status")
        .ok_or("vaadin directory response missing status")?;
    let status = status
        .as_text()
...
mod tests {
    ...
    #[test]
    fn reports_published_for_a_published_component() {
        ...
        let value = resolve_status(&params("vaadinvaadin-grid"), &fetcher).unwrap();
        assert_eq!(value, "published");
    }

    #[test]
    fn reports_unpublished_for_any_other_status() {
        let fetcher = FakeFetcher(
            r#"{"ratingCount": 0, "averageRating": 0, "status": "UNPUBLISHED", "latestAvailableRelease": {"name": "1.0.0", "publicationDate": "2020-01-01T00:00:00Z"}}"#,
        );
        let value = resolve_status(&params("vaadinvaadin-grid"), &fetcher).unwrap();
        assert_eq!(value, "unpublished");
    }
    ...
    fn errors_when_the_field_is_missing() {
        ...
        assert!(resolve_status(&params("vaadinvaadin-grid"), &fetcher).is_err());
    }
}
