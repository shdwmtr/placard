...
pub(crate) fn resolve_stars(
    ...
    let user = params
        .get("user")
        .ok_or("gitea-stars requires a data-user attribute")?;
    let repo = params
        .get("repo")
        .ok_or("gitea-stars requires a data-repo attribute")?;
    ...
    let count = value
        .get("stars_count")
        .ok_or("gitea response missing stars_count")?;
    count
        .as_text()
        .ok_or_else(|| "stars_count was not a plain value".to_string())
}
...
mod tests {
    ...
    #[test]
    fn extracts_stars_count_from_a_gitea_shaped_response() {
        let fetcher = FakeFetcher {
            expected_url: "https://gitea.com/api/v1/repos/gitea/tea",
            body: r#"{"id": 1, "stars_count": 42}"#,
        };
        let value = resolve_stars(&params("gitea", "tea"), &fetcher).unwrap();
        assert_eq!(value, "42");
    }
    ...
    fn requires_user_and_repo_params() {
        ...
        assert!(resolve_stars(&params("gitea", ""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_params_before_fetching() {
        ...
        assert!(resolve_stars(&params("../etc", "tea"), &Unused).is_err());
    }
    ...
    fn errors_when_the_field_is_missing() {
        ...
        assert!(resolve_stars(&params("gitea", "tea"), &fetcher).is_err());
    }
}
