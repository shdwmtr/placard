...
pub(crate) fn resolve_code_size(
    params: &HashMap<String, String>,
    fetcher: &dyn Fetcher,
) -> Result<String, String> {
    let owner = params
        .get("owner")
        .ok_or("github-code-size requires a data-owner attribute")?;
    let repo = params
        .get("repo")
        .ok_or("github-code-size requires a data-repo attribute")?;
...
mod tests {
    ...
    #[test]
    fn sums_all_language_byte_counts() {
        let fetcher = FakeFetcher(r#"{"Python": 39624, "Shell": 104}"#);
        let value = resolve_code_size(&params("badges", "shields"), &fetcher).unwrap();
        assert_eq!(value, "39728");
    }
    ...
    fn requires_owner_and_repo_params() {
        ...
        assert!(resolve_code_size(&HashMap::new(), &Unused).is_err());
        assert!(resolve_code_size(&params("badges", ""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_params_before_fetching() {
        ...
        assert!(resolve_code_size(&params("../etc", "shields"), &Unused).is_err());
    }

    #[test]
    fn errors_when_response_is_not_an_object() {
        let fetcher = FakeFetcher(r#"[1, 2, 3]"#);
        assert!(resolve_code_size(&params("badges", "shields"), &fetcher).is_err());
    }
}
