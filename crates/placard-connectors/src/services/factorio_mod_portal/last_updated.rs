...
pub(crate) fn resolve_last_updated(
    ...
    let mod_name = params
        .get("mod-name")
        .ok_or("factorio-mod-portal-last-updated requires a data-mod-name attribute")?;
    ...
    latest
        ...
        .ok_or_else(|| "factorio mod portal release missing released_at".to_string())
}
...
mod tests {
    ...
    #[test]
    fn extracts_the_latest_release_date() {
        ...
        let value = resolve_last_updated(&params("rso-mod"), &fetcher).unwrap();
        assert_eq!(value, "2021-01-01T00:00:00.000Z");
    }
    ...
    fn rejects_path_breaking_mod_name() {
        ...
        assert!(resolve_last_updated(&params("a?b=c"), &Unused).is_err());
    }

    #[test]
    fn errors_when_releases_are_empty() {
        let fetcher = FakeFetcher {
            expected_url: "https://mods.factorio.com/api/mods/rso-mod",
            body: r#"{"downloads_count": 5, "releases": []}"#,
        };
        assert!(resolve_last_updated(&params("rso-mod"), &fetcher).is_err());
    }
}
