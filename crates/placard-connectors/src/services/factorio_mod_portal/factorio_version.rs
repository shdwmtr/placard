...
pub(crate) fn resolve_factorio_version(
    params: &HashMap<String, String>,
    fetcher: &dyn Fetcher,
) -> Result<String, String> {
    let mod_name = params
        .get("mod-name")
        .ok_or("factorio-mod-portal-factorio-version requires a data-mod-name attribute")?;
    ...
    latest
        .get("info_json.factorio_version")
        .and_then(|v| v.as_text())
        .ok_or_else(|| "factorio mod portal release missing info_json.factorio_version".to_string())
}
...
mod tests {
    ...
    #[test]
    fn extracts_the_latest_release_factorio_version() {
        ...
        let value = resolve_factorio_version(&params("rso-mod"), &fetcher).unwrap();
        assert_eq!(value, "1.1");
    }
    ...
    fn requires_a_mod_name_param() {
        ...
        assert!(resolve_factorio_version(&HashMap::new(), &Unused).is_err());
        assert!(resolve_factorio_version(&params(""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_mod_name() {
        ...
        assert!(resolve_factorio_version(&params("a/b"), &Unused).is_err());
    }

    #[test]
    fn errors_when_info_json_is_missing() {
        let fetcher = FakeFetcher {
            expected_url: "https://mods.factorio.com/api/mods/rso-mod",
            body: r#"{"downloads_count": 5, "releases": [{"version": "1.0.0", "released_at": "2020-01-01T00:00:00.000Z"}]}"#,
        };
        assert!(resolve_factorio_version(&params("rso-mod"), &fetcher).is_err());
    }
}
