...
pub(crate) fn resolve_version(
    ...
    let package_name = params
        .get("package-name")
        .ok_or("aur-version requires a data-package-name attribute")?;
    ...
    let version = first.get("Version").ok_or("aur result missing Version")?;
    version
        .as_text()
...
mod tests {
    ...
    const URL: &str = "https://aur.archlinux.org/rpc?v=5&type=info&arg=visual-studio-code-bin";
    ...
    #[test]
    fn extracts_version_from_an_aur_shaped_response() {
        let fetcher = FakeFetcher(
            URL,
            r#"{"resultcount": 1, "results": [{"Version": "1.85.1-1", "OutOfDate": null}]}"#,
        );
        let value = resolve_version(&params("visual-studio-code-bin"), &fetcher).unwrap();
        assert_eq!(value, "1.85.1-1");
    }
    ...
    fn errors_when_version_is_missing() {
        ...
        assert!(resolve_version(&params("visual-studio-code-bin"), &fetcher).is_err());
    }
    ...
    fn errors_when_package_not_found() {
        ...
        assert!(resolve_version(&params("nonexistent-pkg"), &fetcher).is_err());
    }
}
