...
pub(crate) fn resolve_last_modified(
    params: &HashMap<String, String>,
    fetcher: &dyn Fetcher,
) -> Result<String, String> {
    let package_name = params
        .get("package-name")
        .ok_or("aur-last-modified requires a data-package-name attribute")?;
    ...
    let last_modified = first
        .get("LastModified")
        .ok_or("aur result missing LastModified")?;
    last_modified
        .as_text()
        .ok_or_else(|| "LastModified was not a plain value".to_string())
}
...
mod tests {
    ...
    #[test]
    fn extracts_last_modified_from_an_aur_shaped_response() {
        let fetcher = FakeFetcher(
            URL,
            r#"{"resultcount": 1, "results": [{"LastModified": 1700000000}]}"#,
        );
        let value = resolve_last_modified(&params("google-chrome"), &fetcher).unwrap();
    ...
    fn requires_package_name_param() {
        ...
        assert!(resolve_last_modified(&HashMap::new(), &Unused).is_err());
        assert!(resolve_last_modified(&params(""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_params_before_fetching() {
        ...
        assert!(resolve_last_modified(&params("../etc/passwd"), &Unused).is_err());
    }

    #[test]
    fn errors_when_last_modified_is_missing() {
        let fetcher = FakeFetcher(URL, r#"{"resultcount": 1, "results": [{}]}"#);
        assert!(resolve_last_modified(&params("google-chrome"), &fetcher).is_err());
    }
    ...
    fn errors_when_package_not_found() {
        ...
        assert!(resolve_last_modified(&params("nonexistent-pkg"), &fetcher).is_err());
    }
}
