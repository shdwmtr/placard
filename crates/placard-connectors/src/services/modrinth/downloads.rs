...
pub(crate) fn resolve_downloads(
    ...
    let project_id = params
        .get("project-id")
        .ok_or("modrinth-downloads requires a data-project-id attribute")?;
    ...
    value
        .get("downloads")
        .ok_or("modrinth response missing downloads")?
        .as_text()
...
mod tests {
    ...
    #[test]
    fn extracts_downloads_from_a_modrinth_shaped_response() {
        let fetcher = FakeFetcher {
            expected_url: "https://api.modrinth.com/v2/project/AANobbMI",
            body: r#"{"downloads": 123456, "followers": 42}"#,
        };
        let value = resolve_downloads(&params("AANobbMI"), &fetcher).unwrap();
    ...
    fn errors_when_the_field_is_missing() {
        ...
        assert!(resolve_downloads(&params("AANobbMI"), &fetcher).is_err());
    }
}
