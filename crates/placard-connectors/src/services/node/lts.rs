...
/// comparison against Node's LTS releases (`versionColorForRangeLts`) is
/// badge-drawing logic with no connector equivalent, so it's skipped.
pub(crate) fn resolve_lts(
    params: &HashMap<String, String>,
    fetcher: &dyn Fetcher,
) -> Result<String, String> {
    let package = params
        .get("package")
        .ok_or("node-lts requires a data-package attribute")?;
...
mod tests {
    ...
    fn extracts_engines_node_from_the_latest_version_by_default() {
        ...
        let value = resolve_lts(&params("passport"), &fetcher).unwrap();
    ...
    fn resolves_a_named_tag_through_dist_tags_and_versions() {
        ...
        let value = resolve_lts(&p, &fetcher).unwrap();
        assert_eq!(value, ">=18");
    }
    ...
    fn encodes_scoped_package_names() {
        ...
        let value = resolve_lts(&params("@cedx/gulp-david"), &fetcher).unwrap();
        assert_eq!(value, ">=14");
    }
    ...
    fn requires_package_param() {
        ...
        assert!(resolve_lts(&HashMap::new(), &Unused).is_err());
        assert!(resolve_lts(&params(""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_package_params() {
        ...
        assert!(resolve_lts(&params("../etc/passwd"), &Unused).is_err());
    }
    ...
    fn errors_when_engines_node_is_missing() {
        ...
        assert!(resolve_lts(&params("passport"), &fetcher).is_err());
    }
}
