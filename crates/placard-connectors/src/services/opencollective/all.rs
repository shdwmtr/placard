...
pub(crate) fn resolve_all(
    ...
    let collective = params
        .get("collective")
        .ok_or("opencollective-all requires a data-collective attribute")?;
    let collective = validate_path_param("collective", collective)?;

    let url = format!("https://opencollective.com/{collective}/members/all.json");
...
mod tests {
    ...
    impl Fetcher for FakeFetcher {
        fn fetch(&self, url: &str) -> Result<Vec<u8>, String> {
            assert_eq!(url, "https://opencollective.com/shields/members/all.json");
    ...
    #[test]
    fn counts_members_with_backer_role() {
        let fetcher = FakeFetcher(
            r#"[{"MemberId": 1, "type": "USER", "role": "BACKER"}, {"MemberId": 2, "type": "ORGANIZATION", "role": "BACKER"}, {"MemberId": 3, "type": "USER", "role": "ADMIN"}]"#,
        );
        let value = resolve_all(&params("shields"), &fetcher).unwrap();
        assert_eq!(value, "2");
    }
    ...
    fn requires_collective_param() {
        ...
        assert!(resolve_all(&HashMap::new(), &Unused).is_err());
        assert!(resolve_all(&params(""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_params_before_fetching() {
        ...
        assert!(resolve_all(&params("../etc"), &Unused).is_err());
    }

    #[test]
    fn errors_when_response_is_not_an_array() {
        let fetcher = FakeFetcher(r#"{"error": "not found"}"#);
        assert!(resolve_all(&params("shields"), &fetcher).is_err());
    }
}
