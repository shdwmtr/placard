...
pub(crate) fn resolve_sponsors(
    params: &HashMap<String, String>,
    fetcher: &dyn Fetcher,
) -> Result<String, String> {
    let collective = params
        .get("collective")
        .ok_or("opencollective-sponsors requires a data-collective attribute")?;
    let collective = validate_path_param("collective", collective)?;

    let url = format!("https://opencollective.com/{collective}/members/organizations.json");
...
mod tests {
    ...
    impl Fetcher for FakeFetcher {
        fn fetch(&self, url: &str) -> Result<Vec<u8>, String> {
            assert_eq!(
                url,
                "https://opencollective.com/shields/members/organizations.json"
            );
    ...
    #[test]
    fn counts_organization_members_with_backer_role() {
        let fetcher = FakeFetcher(
            r#"[{"MemberId": 1, "type": "ORGANIZATION", "role": "BACKER"}, {"MemberId": 2, "type": "ORGANIZATION", "role": "ADMIN"}]"#,
        );
        let value = resolve_sponsors(&params("shields"), &fetcher).unwrap();
        assert_eq!(value, "1");
    }

    #[test]
    fn returns_zero_for_no_sponsors() {
        let fetcher = FakeFetcher("[]");
        let value = resolve_sponsors(&params("shields"), &fetcher).unwrap();
        assert_eq!(value, "0");
    }
    ...
    fn requires_collective_param() {
        ...
        assert!(resolve_sponsors(&HashMap::new(), &Unused).is_err());
        assert!(resolve_sponsors(&params(""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_params_before_fetching() {
        ...
        assert!(resolve_sponsors(&params("../etc"), &Unused).is_err());
    }

    #[test]
    fn errors_when_response_is_not_an_array() {
        let fetcher = FakeFetcher(r#"{"error": "not found"}"#);
        assert!(resolve_sponsors(&params("shields"), &fetcher).is_err());
    }
}
