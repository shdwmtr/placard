...
pub(crate) fn resolve_rating_count(
    ...
    let store_id = params
        .get("store-id")
        .ok_or("chrome-web-store-rating-count requires a data-store-id attribute")?;
    ...
    let pos = find_class_open_tag_end(&html, "xJEoWe")
        .ok_or("chrome web store page did not contain a rating count element")?;
    let text = text_immediately_after(&html, pos)
        .ok_or_else(|| "chrome web store rating count element was empty".to_string())?;
    let count = text.strip_suffix("ratings").unwrap_or(&text).trim();
    if count.is_empty() {
        return Err("chrome web store rating count element was empty".to_string());
    }
    Ok(count.to_string())
}
...
mod tests {
    ...
    #[test]
    fn extracts_and_strips_the_ratings_suffix() {
        let html = r#"<a href="./reviews"><p class="xJEoWe">25 ratings</p></a>"#;
        let value = resolve_rating_count(
    ...
    #[test]
    fn errors_when_the_rating_count_element_is_missing() {
        let html = r#"<div class="other">nothing here</div>"#;
        assert!(
            resolve_rating_count(
                &params("ogffaloegjglncjfehdfplabnoondfjo"),
                &FakeFetcher(html)
            )
            .is_err()
