...
fn project_url(params: &HashMap<String, String>) -> Result<String, String> {
    let platform = params
        .get("platform")
        .ok_or("librariesio-dependents requires a data-platform attribute")?;
    let package_name = params
        .get("package-name")
        .ok_or("librariesio-dependents requires a data-package-name attribute")?;
...
pub(crate) fn resolve_dependents(
    ...
    value
        ...
        .ok_or("libraries.io response missing dependents_count")?
        .as_text()
        .ok_or_else(|| "dependents_count was not a plain value".to_string())
}
...
mod tests {
    ...
    fn extracts_dependents_count() {
        ...
        let value = resolve_dependents(&params("npm", "lodash"), &fetcher).unwrap();
        assert_eq!(value, "133226");
    }
    ...
    fn builds_the_url_with_a_scope() {
        ...
        let value = resolve_dependents(&p, &fetcher).unwrap();
        assert_eq!(value, "5");
    }
    ...
    fn requires_platform_and_package_name_params() {
        ...
        assert!(resolve_dependents(&params("npm", ""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_params_before_fetching() {
        ...
        assert!(resolve_dependents(&params("../etc", "lodash"), &Unused).is_err());
    }
    ...
    fn errors_when_the_field_is_missing() {
        ...
        assert!(resolve_dependents(&params("npm", "lodash"), &fetcher).is_err());
    }
}
