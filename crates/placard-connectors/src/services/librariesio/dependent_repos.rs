...
fn project_url(params: &HashMap<String, String>) -> Result<String, String> {
    let platform = params
        .get("platform")
        .ok_or("librariesio-dependent-repos requires a data-platform attribute")?;
    let package_name = params
        .get("package-name")
        .ok_or("librariesio-dependent-repos requires a data-package-name attribute")?;
...
pub(crate) fn resolve_dependent_repos(
    ...
    value
        .get("dependent_repos_count")
        .ok_or("libraries.io response missing dependent_repos_count")?
        .as_text()
        .ok_or_else(|| "dependent_repos_count was not a plain value".to_string())
}
...
mod tests {
    ...
    #[test]
    fn extracts_dependent_repos_count() {
        ...
        let value = resolve_dependent_repos(&params("npm", "lodash"), &fetcher).unwrap();
        assert_eq!(value, "44640");
    }
    ...
    fn builds_the_url_with_a_scope() {
        ...
        let value = resolve_dependent_repos(&p, &fetcher).unwrap();
        assert_eq!(value, "10");
    }
    ...
    fn requires_platform_and_package_name_params() {
        ...
        assert!(resolve_dependent_repos(&HashMap::new(), &Unused).is_err());
        assert!(resolve_dependent_repos(&params("npm", ""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_params_before_fetching() {
        ...
        assert!(resolve_dependent_repos(&params("../etc", "lodash"), &Unused).is_err());
    }
    ...
    fn errors_when_the_field_is_missing() {
        ...
        assert!(resolve_dependent_repos(&params("npm", "lodash"), &fetcher).is_err());
    }
}
