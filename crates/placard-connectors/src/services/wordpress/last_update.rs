...
pub(crate) fn resolve_last_update(
    ...
    if extension_type != "plugin" && extension_type != "theme" {
        return Err(
            "wordpress-last-update 'type' parameter must be 'plugin' or 'theme'".to_string(),
        );
    }
    let slug = params
        .get("slug")
        .ok_or("wordpress-last-update requires a data-slug attribute")?;
    ...
    let last_updated = value
        .get("last_updated")
        .ok_or("wordpress response missing last_updated")?;
    last_updated
        .as_text()
        .ok_or_else(|| "last_updated was not a plain value".to_string())
}
...
mod tests {
    ...
    #[test]
    fn extracts_last_updated_field() {
        let fetcher = FakeFetcher(
            ...
            r#"{"version": "2.6.9", "last_updated": "2024-01-15 3:45am GMT"}"#,
        );
        let value = resolve_last_update(&params("plugin", "bbpress"), &fetcher).unwrap();
        assert_eq!(value, "2024-01-15 3:45am GMT");
    }
    ...
    fn uses_theme_endpoint_for_theme_type() {
        let fetcher = FakeFetcher(
            ...
            r#"{"version": "3.0", "last_updated": "2024-02-01"}"#,
        );
        let value = resolve_last_update(&params("theme", "twentyseventeen"), &fetcher).unwrap();
        assert_eq!(value, "2024-02-01");
    }
    ...
    fn requires_slug_param() {
        ...
        assert!(resolve_last_update(&params("plugin", ""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_params_before_fetching() {
        ...
        assert!(resolve_last_update(&params("plugin", "../etc"), &Unused).is_err());
    }

    #[test]
    fn errors_when_last_updated_field_is_missing() {
        let fetcher = FakeFetcher(
            "https://api.wordpress.org/plugins/info/1.2/?action=plugin_information&request[slug]=bbpress&request[fields][active_installs]=1&request[fields][sections]=0&request[fields][homepage]=0&request[fields][tags]=0&request[fields][screenshot_url]=0&request[fields][downloaded]=1&request[fields][last_updated]=1&request[fields][requires_php]=1",
            r#"{"version": "2.6.9"}"#,
        );
        assert!(resolve_last_update(&params("plugin", "bbpress"), &fetcher).is_err());
    }
}
