...
pub(crate) fn resolve_rating(
    ...
    if extension_type != "plugin" && extension_type != "theme" {
        return Err("wordpress-rating 'type' parameter must be 'plugin' or 'theme'".to_string());
    }
    let slug = params
        .get("slug")
        .ok_or("wordpress-rating requires a data-slug attribute")?;
    ...
    let rating = match value.get("rating") {
        Some(Value::Number(n)) => *n,
        _ => return Err("wordpress response missing rating".to_string()),
    };
    let scaled = (rating / 100.0) * 5.0;
    Ok(format!("{scaled:.1}"))
}
...
mod tests {
    ...
    #[test]
    fn scales_rating_to_a_five_star_value() {
        ...
        let value = resolve_rating(&params("plugin", "bbpress"), &fetcher).unwrap();
        assert_eq!(value, "4.5");
    }
    ...
    fn requires_slug_param() {
        ...
        assert!(resolve_rating(&params("plugin", ""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_params_before_fetching() {
        ...
        assert!(resolve_rating(&params("plugin", "../etc"), &Unused).is_err());
    }
    ...
    fn errors_when_rating_field_is_missing() {
        ...
        assert!(resolve_rating(&params("plugin", "bbpress"), &fetcher).is_err());
    }
}
