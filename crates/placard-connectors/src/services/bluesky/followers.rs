...
pub(crate) fn resolve_followers(
    ...
    let actor = params
        .get("actor")
        .ok_or("bluesky-followers requires a data-actor attribute")?;
    ...
    let followers = value
        .get("followersCount")
        .ok_or("bluesky response missing followersCount")?;
    followers
        .as_text()
        .ok_or_else(|| "followersCount was not a plain value".to_string())
}
...
mod tests {
    ...
    #[test]
    fn extracts_followers_count_from_a_bluesky_shaped_response() {
        let fetcher = FakeFetcher(
            r#"{"did":"did:plc:abc","handle":"chitvs.bsky.social","followersCount":1234,"postsCount":56}"#,
        );
        let value = resolve_followers(&params("chitvs.bsky.social"), &fetcher).unwrap();
    ...
    fn errors_when_the_field_is_missing() {
        ...
        assert!(resolve_followers(&params("chitvs.bsky.social"), &fetcher).is_err());
    }
}
