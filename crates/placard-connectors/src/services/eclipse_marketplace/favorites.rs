...
pub(crate) fn resolve_favorites(
    params: &HashMap<String, String>,
    fetcher: &dyn Fetcher,
) -> Result<String, String> {
    let name = params
        .get("name")
        .ok_or("eclipse-marketplace-favorites requires a data-name attribute")?;
    ...
    extract_xml_field(&text, "favorited")
        .ok_or_else(|| "eclipse marketplace response missing favorited".to_string())
}
...
mod tests {
    ...
    #[test]
    fn extracts_favorited_count_from_an_eclipse_marketplace_shaped_response() {
        let fetcher = FakeFetcher(SAMPLE);
        let value = resolve_favorites(&params("notepad4e"), &fetcher).unwrap();
        assert_eq!(value, "10");
    }
    ...
    fn requires_name_param() {
        ...
        assert!(resolve_favorites(&HashMap::new(), &Unused).is_err());
        assert!(resolve_favorites(&params(""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_params_before_fetching() {
        ...
        assert!(resolve_favorites(&params("../etc/passwd"), &Unused).is_err());
    }

    #[test]
    fn errors_when_the_field_is_missing() {
        let fetcher = FakeFetcher(r#"<marketplace><node id="595"></node></marketplace>"#);
        assert!(resolve_favorites(&params("notepad4e"), &fetcher).is_err());
    }
}
