...
pub(crate) fn resolve_version(
    ...
    let name = params
        .get("name")
        .ok_or("eclipse-marketplace-version requires a data-name attribute")?;
    ...
    extract_xml_field(&text, "version")
        .ok_or_else(|| "eclipse marketplace response missing version".to_string())
}
...
mod tests {
    ...
    #[test]
    fn extracts_version_from_an_eclipse_marketplace_shaped_response() {
        let fetcher = FakeFetcher(SAMPLE);
        let value = resolve_version(&params("notepad4e"), &fetcher).unwrap();
    ...
    fn errors_when_the_field_is_missing() {
        ...
        assert!(resolve_version(&params("notepad4e"), &fetcher).is_err());
    }
}
