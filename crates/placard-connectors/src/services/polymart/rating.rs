...
pub(crate) fn resolve_rating(
    ...
    let resource_id = params
        .get("resource-id")
        .ok_or("polymart-rating requires a data-resource-id attribute")?;
    ...
    let average = match value.get("response.resource.reviews.stars") {
        ...
        _ => return Err("polymart response missing response.resource.reviews.stars".to_string()),
    };
    let count = match value.get("response.resource.reviews.count") {
        Some(Value::Number(n)) => *n,
        _ => return Err("polymart response missing response.resource.reviews.count".to_string()),
    };
