...
pub(crate) fn resolve_watchers(
    ...
    let slug = params
        .get("slug")
        .ok_or("hangar-watchers requires a data-slug attribute")?;
    ...
    let watchers = value
        .get("stats.watchers")
        .ok_or("hangar response missing stats.watchers")?;
    watchers
        .as_text()
        .ok_or_else(|| "stats.watchers was not a plain value".to_string())
}
...
mod tests {
    ...
    #[test]
    fn extracts_watchers_from_a_hangar_shaped_response() {
        let fetcher = FakeFetcher(
            r#"{"stats": {"views": 1, "downloads": 2, "recentViews": 3, "recentDownloads": 4, "stars": 5, "watchers": 99}}"#,
        );
        let value = resolve_watchers(&params("Essentials"), &fetcher).unwrap();
        assert_eq!(value, "99");
    }
    ...
    fn requires_slug_param() {
        ...
        assert!(resolve_watchers(&params(""), &Unused).is_err());
    }
    ...
    fn rejects_path_breaking_params_before_fetching() {
        ...
        assert!(resolve_watchers(&params("../etc/passwd"), &Unused).is_err());
    }

    #[test]
    fn errors_when_the_field_is_missing() {
        let fetcher = FakeFetcher(r#"{"stats": {"views": 1}}"#);
        assert!(resolve_watchers(&params("Essentials"), &fetcher).is_err());
    }
}
