...
pub(crate) fn resolve_downloads(
    ...
    let slug = params
        .get("slug")
        .ok_or("hangar-downloads requires a data-slug attribute")?;
    ...
    let downloads = value
        .get("stats.downloads")
        .ok_or("hangar response missing stats.downloads")?;
    downloads
        .as_text()
        .ok_or_else(|| "stats.downloads was not a plain value".to_string())
}
...
mod tests {
    ...
    #[test]
    fn extracts_downloads_from_a_hangar_shaped_response() {
        let fetcher = FakeFetcher(
            r#"{"stats": {"views": 1, "downloads": 55000, "recentViews": 3, "recentDownloads": 4, "stars": 5, "watchers": 6}}"#,
        );
        let value = resolve_downloads(&params("Essentials"), &fetcher).unwrap();
    ...
    fn errors_when_the_field_is_missing() {
        ...
        assert!(resolve_downloads(&params("Essentials"), &fetcher).is_err());
    }
}
